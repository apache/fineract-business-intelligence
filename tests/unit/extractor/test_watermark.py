...
def test_get_returns_stored_cursor_value():
    stored = datetime(2026, 8, 1, 10, 0, tzinfo=timezone.utc)
...
def test_update_never_writes_outside_its_own_tenant():
    ...
    manager.update("m_loan", "last_modified_on_utc", datetime.now(timezone.utc))
...
def test_incremental_subtracts_the_lookback_window_from_the_watermark(app_config):
    stored = datetime(2026, 8, 2, 12, 0, tzinfo=timezone.utc)

    _, cursor, _ = _run_extract(app_config, "incremental", stored, batches=[])

    _, params = cursor.executed[0]
    assert params == (datetime(2026, 8, 2, 11, 50, tzinfo=timezone.utc),)
...
def test_watermark_advances_to_the_max_cursor_value_in_each_batch(app_config):
    older = datetime(2026, 8, 1, 8, 0, tzinfo=timezone.utc)
    newer = datetime(2026, 8, 1, 9, 0, tzinfo=timezone.utc)
...
def test_row_count_accumulates_across_multiple_batches(app_config):
    row = _row_with_cursor(datetime(2026, 8, 1, 8, 0, tzinfo=timezone.utc))
    batches = [[row] * 3, [row] * 2, []]
