...
principal_outstanding_as_of as (
    ...
    left join lateral (
        select tb.outstanding_loan_balance_derived
        from {{ ref('stg_m_loan_transaction') }} tb
        where
            tb.tenant_id = ls.tenant_id
...
latest_obligation_transaction as (
    ...
    from loan_snapshots ls
    left join lateral (
        select t.loan_transaction_id
        from {{ ref('stg_m_loan_transaction') }} t
        where
            t.tenant_id = ls.tenant_id
